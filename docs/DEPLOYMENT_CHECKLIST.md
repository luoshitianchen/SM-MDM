# 主数据管理（MDM）服务 上线部署检查清单（DEPLOYMENT_CHECKLIST）
...
- [ ] ConfigMap 环境变量正确（SM_ENV / SM_DATABASE_NAME=sm_mdm）
...
1. `helm lint helm/sm-mdm`
2. `helm upgrade --install sm-mdm helm/sm-mdm -n sm-prod -f helm/sm-mdm/values-prod.yaml`
3. 等待 rollout：`kubectl -n sm-prod rollout status deploy/sm-mdm`
